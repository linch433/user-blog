...
function Modal({active, closeModal, children, width = 'w-3/4', height = 'h-3/4', roundStyle, isSidePanel}) {
    ...
    return (
        <>
            {active && (
                <div
                    className='flex items-center justify-center fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-60 z-20'>
                    <div
                        ref={container}
                        className={
                            isSidePanel
                                ? `bg-[#fafafa] h-screen w-[50vw] absolute right-0 overflow-auto`
                                : `bg-white ${width} ${height} ${
                                    roundStyle ? roundStyle : 'rounded-2xl'
                                } 
